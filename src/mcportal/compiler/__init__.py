...
2. :mod:`~mcportal.compiler.inference` — 정규화 응답 샘플 3~5개에서 JSON Schema 를
   추론한다(오프라인·결정론·입력 순서 무관).
3. :mod:`~mcportal.compiler.openapi` — ``SourceSpec``(+추론 스키마)을 OpenAPI 3.1
   문서로 컴파일하고 결정론 직렬화한다.
4. :mod:`~mcportal.compiler.sampler` — 라이브 샘플링을 쿼터가드 경유로 오케스트레이션
   한다(하드캡 5회·스크러빙 강제).

여기서 재수출하는 이름들은 :mod:`mcportal` 최상위가 그대로 다시 올린다. 이름을
바꾸면 최상위 공개 API 가 함께 흔들리므로 철자를 고정한다.
"""
